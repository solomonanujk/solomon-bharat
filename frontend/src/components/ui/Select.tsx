...
function SelectValue({ placeholder }: { placeholder?: string }) {
  ...
  return (
    <span className={ctx.value ? 'text-text-primary' : 'text-text-muted/60'}>
...
function SelectTrigger({ className, children, ...props }: SelectTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'h-10 w-full px-3 border border-border rounded bg-bg-surface',
        'text-sm font-sans text-text-primary',
        'flex items-center justify-between cursor-pointer',
        'hover:border-text-primary/30 transition-colors',
    ...
    >
      ...
      <ChevronDown
        size={16}
        className={cn('text-text-muted ml-2 flex-shrink-0 transition-transform duration-200', open && 'rotate-180')}
        aria-hidden="true"
      />
    </button>
  );
...
function SelectContent({ className, children, ...props }: SelectContentProps) {
  ...
  return (
    <div
      role="listbox"
      className={cn(
        'absolute mt-1 bg-bg-surface border border-border rounded',
...
function SelectItem({ value, className, children, ...props }: SelectItemProps) {
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={cn(
        'px-3 py-2 text-sm font-sans text-text-primary cursor-pointer',
        'flex items-center justify-between',
        'hover:bg-fill-subtle transition-colors',
        isSelected && 'bg-fill-subtle font-semibold',
        className
      )}
      {...props}
    >
      {children}
      {isSelected && <Check size={14} className="text-accent-primary ml-2 flex-shrink-0" aria-hidden="true" />}
    </div>
  );
}
