...
function KpiCard({
  ...
  return (
    <div className="bg-surface border border-border-warm rounded p-5 flex items-center gap-4">
      ...
      <div>
        <p className="text-[22px] font-[700] font-sans text-primary leading-none">
          {loading ? '—' : value}
        </p>
        <p className="text-[12px] font-sans text-muted-text mt-1">{label}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <AccountPageWrapper title="Dashboard" description={`Welcome back, ${firstName}.`}>
      ...
      <div className="bg-surface border border-border-warm rounded p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <p className="text-[16px] font-[700] font-sans text-primary">Ready to source more?</p>
      ...
      <div className="border border-border-warm rounded bg-surface overflow-hidden">
        <div className="px-5 py-4 border-b border-border-warm flex items-center justify-between">
          <h2 className="text-[15px] font-[700] font-sans text-primary">Recent Orders</h2>
          <Link href="/orders" className="text-[12px] font-[700] font-sans text-accent hover:text-accent-hover transition-colors">
            View all
          </Link>
        </div>
        ...
        {isLoading ? (
          ...
          <p className="px-5 py-8 text-center text-[13px] font-sans text-muted-text">
            No orders yet.
          </p>
        ) : (
          <div className="divide-y divide-border-warm">
            {data.recentOrders.map((order) => (
              ...
              >
                <div>
                  <p className="text-[13px] font-[700] font-sans text-primary">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className="text-[13px] font-[700] font-sans text-primary">
