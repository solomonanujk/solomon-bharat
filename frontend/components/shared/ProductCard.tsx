  ...
  return (
    <div className={cn('group flex flex-col', className)}>
      ...
      <Link href={`/products/${slug}`} className="relative block aspect-square overflow-hidden rounded-sm bg-[#F0EBE3]">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1.5">
          <button
            ...
            className="w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow-sm transition-colors duration-150 disabled:opacity-60"
          >
            <Heart
              size={14}
        ...
        {cartItem ? (
          <div className="absolute bottom-2 right-2 group/qty">
            <div className="flex items-center justify-center w-9 h-9 rounded-full bg-primary text-white text-[13px] font-[700] font-public-sans shadow-sm group-hover/qty:hidden">
              {cartItem.quantity}
            </div>
            <div className="hidden group-hover/qty:flex items-center gap-0.5 h-9 pl-1 pr-1 rounded-full bg-white shadow-sm border border-border-warm">
              ...
              <span className="text-[13px] font-[700] font-public-sans text-primary min-w-[16px] text-center">
                {cartItem.quantity}
              </span>
              <button
                type="button"
      ...
      <div className="mt-2.5 flex flex-col gap-1">
        <div className="text-[20px] font-[700] font-public-sans text-primary leading-none">
          <Price amountInr={price} size="md" className="!text-[20px] !font-[700]" />
        </div>

        <Link
          href={`/products/${slug}`}
          className="text-[16px] font-[700] font-public-sans text-primary leading-snug line-clamp-2 tracking-[0.02em] hover:underline"
        >
          {name}
        </Link>
