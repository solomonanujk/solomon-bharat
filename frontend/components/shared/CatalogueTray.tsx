...
export function CatalogueTray() {
  ...
  return (
    ...
    >
      ...
      <p className="text-[13px] font-[700] font-public-sans flex-1">
        {items.length} {items.length === 1 ? 'product' : 'products'} in catalogue
      </p>
      <Link
        href="/catalogue"
        onClick={hideTray}
        className="text-[13px] font-[700] font-public-sans bg-white text-primary rounded-full px-3 py-1.5 hover:bg-white/90 transition-colors"
      >
        Set price &amp; generate
      </Link>
    </div>
